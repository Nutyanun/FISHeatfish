...
public partial class Player : CharacterBody2D
{
	...
	[Export] public float BiteReach = 90f; // วัดจากปาก
	...
	[Export] public float SpawnGraceSeconds { get; set; } = 1.2f;
	private float _spawnGraceTimer = 0f;

	// ดีบัก: กัดอัตโนมัติเมื่อกัดได้ (ช่วยทดสอบ collision)
	[Export] public bool AutoBiteWhenPossible = true;
	[Export] public bool DebugBypassBiteChecks = true;

	// กัน Hurt ระยะไกลเกินจริง (ใช้กับตาย ไม่ได้ใช้กับกัดแล้ว)
	[Export] public float HurtTriggerDistance = 28f;

	// กันเก็บคริสตัลซ้ำ (ทั้ง BodyEntered/AreaEntered)
	...
	[Export] public int MaxShieldStacks = 99;
	...
	[Export] public float CrystalPickupRadius = 60f;

	// ===== Utils หา Manager =====
	private ScoreManager GetSM() => ScoreManager.Instance;
	private SkillManager GetSKM()
	{
		var skm = GetNodeOrNull<SkillManager>("%SkillManager");
	...
	private bool IsCrystalNode(Node n)
	{
		...
		if (n.IsInGroup("Crystal") || n is CrystalPickup) return true;
		var nm = n.Name.ToString().ToLowerInvariant();
		return nm.Contains("crystal") || nm.Contains("gem") || nm.Contains("pickup");
	}
	...
	// DEFERRED collect to avoid "flushing_queries" error
	private void EatCrystal(CrystalPickup cp)
	{
		if (cp != null && IsInstanceValid(cp))
			cp.CallDeferred("CollectBy", this); // เดิม cp.CollectBy(this);
	}

	// helper to Apply skill via SkillManager in a deferred call
	...
	private void OnHurtEnter(Node other)
{
	...
	private void UpdateSizeBasedOnScore(ScoreManager sm)
	{
		...
		if (sm.LevelScore >= 30) targetScale = 2.4f;
		else if (sm.LevelScore >= 15) targetScale = 1.9f;
