using Godot;                                       // ใช้คลาสของ Godot เช่น TextureButton, LineEdit, Color, Vector2
using System;                                      // ใช้ .NET พื้นฐาน (ไม่ได้ใช้เยอะในไฟล์นี้ แต่เผื่อไว้)

public partial class EyeButton : TextureButton     // คลาสปุ่มรูปตา สืบทอดจาก TextureButton (ปุ่มที่มีภาพ)
{
	...
	// ถ้าไม่อยากให้ภาพจางตอนซ่อน ให้ตั้ง HiddenOpacity = 1.0 ใน Inspector
	[Export(PropertyHint.Range, "0,1,0.05")] public float HiddenOpacity = 0.6f; 
	// [Export] ตัวเลข 0–1 (แสดงเป็น slider) ใช้กำหนดความทึบเมื่อซ่อนรหัส เช่น 0.6 = จางหน่อย

	private LineEdit _line;                         // ตัวแปรเก็บอ้างอิงช่องรหัสผ่าน (LineEdit)
	private bool _hidden;                           // เก็บสถานะว่าขณะนี้ "ซ่อนรหัสอยู่หรือไม่"

	public override void _Ready()                   // เรียกเมื่อ Node พร้อมใช้งาน (หลังโหลดเข้าฉาก)
	{
		...
		// ===== ตั้งค่าขนาดปุ่ม =====
		CustomMinimumSize = new Vector2(20, 20);    // ขนาดขั้นต่ำ 20×20 พิกเซล (ปรับเป็น 16/24 ได้)
	...
	private void OnPressed()                        // เรียกเมื่อผู้ใช้ "คลิกปุ่มตา"
	{
	...
	private void Apply()                            // ฟังก์ชันหลักที่อัปเดตสถานะของช่องรหัสและปุ่ม
	{
